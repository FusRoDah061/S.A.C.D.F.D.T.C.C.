...
//FUNÇŐES CORRELATAS As ESTRUTURAS;
/*
...
void calcula_ferro(float perimetro, float profundidade, float altura, int qtde_colunas, float largura_baldrame, float profundidade_baldrame)
{
	...
	do{
		...
		printf("Digite o espaçamento entre as amarraçőes do estribo: ");
		scanf("%f", &espacamento);
    ...
    do
    {
        ...
        switch(opt)
        {
            ...
            case 1:
                ...
                printf("\nInforme as dimensőes da telha\n");

                float a_largura, a_comprimento, area_telha;

                //Para telhas de amianto ou alumínio as dimensőes podem variar, por isso o usuário é quem entra com essas dimensőes
...
/*
 * Calcula a quantidade total de galőes de tinta necessários para pintar a área interna da construção
 ...
 *
 * Retorna:
 * (int) qtde_total_galoes: quantidade total de galőes de tinta necessários para pintar a área interna da construção.
 */
...
/*
 * Exibe opçőes de revestimento para que o usuário possa escolher o revestimento que
 ...
 * oitőes e na quantidade de tiolos necessários para preencher uma área de um metro quadrado.
 * Armaneza a quantidade de tijolos necessários por oitão em uma variável responsável por
 * contabilizar a quantidade total de tijolos necessários para construir todas as paredes e oitőes
 * da construção.
 *
 * Parâmetros:
 * (int) qtde_tijolos_por_oitão: vetor que armazena em cada posição a quantidade de tijolos necessária para construir um oitão.
 * (int) qtde_oitoes: quantidade total de oitőes presentes na construção.
 * (int) qtde_total_tijolos: variável que contabiliza a quantidade total de tijolos necessária para toda a construção.
 * (float) areas_oitőes: vetor que armazena em casa posição a área de cada oitão individualmente.
...
/*
 * Exibe opçőes de tijolos para que o usuário possa escolher o tijolo que
 * deseja utilizar em sua obra.
 *
 * Retorna:
...
void novo_orcamento()
{
    ...
    //Geral
    int usr_opt = 0, //Usada para receber as escolhas do usuário quando forem apresentadas opçőes
    	chave_mestra = 0;
    ...
    int tipo_tijolo = 0, //Armazena o tipo de tijolo selecionado pelo usuário.
        ...
        qtde_oitoes = 0, //Quantidade total de oitőes presentes na construção.
	    ...
	    do {
	        printf("\nDigite a quantidade de oitőes presentes em sua casa: ");
	        scanf("%d", &qtde_oitoes);
	    } while(qtde_oitoes <= 0);
	...
	do {
	    ...
	    printf("\n\n----- Quantidade total de galőes de tinta -----");
	    printf("\n %d galőes de tinta", g_qtde_total_galoes_tinta);
